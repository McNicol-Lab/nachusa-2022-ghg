...
july_aug_modified_data %>%
  #filter(gas == "co2") %>%
  ggplot(aes(x = factor(date.x), y = flux)) +
  geom_violin() +
  facet_grid(.~gas, labeller = labeller(gas = gas.labs)) +
  labs(title = "GHG fluxes across sampling dates", x = "Date", y = expression( "Flux ("*mu*"/nmol GHG m"^{-2}*" s"^{-1}*")" ) )+
  theme_bw()
...
july_aug_modified_data %>%
  ...
  geom_boxplot() +
  facet_grid(.~gas, labeller = labeller(gas = gas.labs)) +
  labs(title = "GHG fluxes across transect positions", x = "Position", y = "Flux (mg/mug GHG m-2 d-1)")+
  theme_bw()
...
july_aug_modified_data %>%
  ...
  ggplot(aes(x = transect, y = flux)) +
  geom_boxplot() +
  facet_grid(ecosystem~gas, labeller = labeller(gas = gas.labs, ecosystem = eco.labs)) +
  labs(title = "GHG fluxes across transect positions", x = "Position", y = "Flux (mg/mug GHG m-2 d-1)")+
  theme_bw()
...
july_aug_modified_data %>%
  ...
  labs(title = "CO2 Fluxes vs. Soil Water Content", y = "Flux (mg CO2 m-2 d-1)", x = "Water Content (%)") +
  scale_color_viridis_d(labels = eco.labs) +
...
july_aug_modified_data %>%
  ...
  geom_smooth(method = "lm", color = "black") +
  #facet_grid(.~date.x, labeller = labeller(date.x = date.labs)) +
  labs(title = "CH4 Fluxes vs. Soil Water Content", y = "Flux (mug CH4 m-2 d-1)", x = "Water Content (%)") +
  scale_color_viridis_d(labels = eco.labs) +
  guides(guide_legend(title = "Ecosystem"))+
  theme_bw()
...
july_aug_modified_data %>%
  filter(gas == "co2", date.x = "8/18/2022") %>%
  ggplot(aes(soil_temperature, flux, color = ecosystem)) +
  geom_point() +
  geom_smooth(method = "lm", color = "black") +
  facet_grid(.~date.x) +
  labs(title = "CO2 Fluxes vs. Average Temperature", y = "Flux (mg CO2 m-2 d-1)", x = "Temperature (K)") +
  scale_color_viridis_d(labels = eco.labs) +
  guides(guide_legend(title = "Ecosystem"))+
  theme_bw()
...
july_aug_modified_data %>%
  ...
  geom_smooth(method = "lm", color = "black") +
  facet_grid(.~date.x) +
  labs(title = "CH4 Fluxes vs. Average Temperature", y = "Flux (mug CH4 m-2 d-1)", x = "Temperature (K)") +
  scale_color_viridis_d(labels = eco.labs) +
  guides(guide_legend(title = "Ecosystem"))+
  theme_bw()
