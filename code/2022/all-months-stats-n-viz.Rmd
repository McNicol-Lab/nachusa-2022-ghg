...
```{r add-eco-variable}
full_data %>%
  mutate(eco = case_when(ecosystem == "BP" ~ "prairie",
                         ecosystem == "SP" ~ "prairie",
                         ecosystem == "EW" ~ "woodland",
                         ecosystem == "WW" ~ "woodland",
                         ecosystem == "SW" ~ "wetland",
                         ecosystem == "NW" ~ "wetland",
                         .default = as.character(ecosystem))) %>%
  write_csv("data/2022_full_data.csv")
...
```{r read-csv}
mod_data <- read_csv("data/2022_full_dataset.csv")
```
...
```{r metadata-for-microbes}
full_data %>%
  filter(date=="2022-09-24") %>%
  mutate(chamber=paste(ecosystem,rep,sep="-")) %>%
  select(12,8,3,6,7,10,11,4,5,9,1,2) %>%
  write_csv("data/2022_full_metadata.csv")

full_data %>%
  filter(date=="2022-09-24") %>%
  mutate(chamber=paste(ecosystem,rep,sep="-")) %>%
  select(12,8,3,6,7,10,11,4,5,9,1,2) %>%
  filter(chamber %in% c("WW-1","WW-2","WW-3","WW-4","WW-5","WW-9","BP-1","BP-2","BP-3","BP-4","BP-5","BP-9","SW-1","SW-2","SW-3","SW-4","SW-5","SW-9","EW-1","EW-5","EW-9","NW-1","NW-5","NW-9","SP-1","SP-5","SP-9"))%>%
write_csv("data/2022_metadata_samples_only.csv")
