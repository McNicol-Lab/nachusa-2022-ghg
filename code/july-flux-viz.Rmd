...
fluxes <- read_csv("data/220712_fluxes.csv")
...
full_data <- left_join(fluxes, moisture_july, by = c("ecosystem", "rep"))
...
ppm_data<-bind_rows(read_csv("data/220712_t0_ghg_ppm.csv"),
          read_csv("data/220712_t1_ghg_ppm.csv"),
          read_csv("data/220712_t2_ghg_ppm.csv"),
          read_csv("data/220712_t3_ghg_ppm.csv"),
          read_csv("data/220712_t4_ghg_ppm.csv"))
```
...
full_data %>% 
  ...
  geom_point() +
  labs(title = "CH4 fluxes vs. Gravimetric Soil Water Content", y = "Flux (μg CH4 m-2 d-1)", x = "Water Content (%)") +
...
full_data %>% 
  ...
  ggplot(aes(x = soil_moisture, y = flux, color = factor(rep))) +
  geom_point() +
  labs(title = "N2O fluxes vs. Gravimetric Soil Water Content", y = "Flux (μg N2O m-2 d-1)", x = "Water Content (%)") +
  facet_wrap(~ecosystem) +
  guides(color = guide_legend(title = "Chamber ID"))
...
full_data %>% 
  ...
  ggplot(aes(x = soil_moisture, y = flux, color = factor(rep))) +
  geom_point() +
  labs(title = "CO2 fluxes vs. Gravimetric Soil Water Content", y = "Flux (mg CO2 m-2 d-1)", x = "Water Content (%)") +
  facet_wrap(~ecosystem) +
  guides(color = guide_legend(title = "Chamber ID"))
```
